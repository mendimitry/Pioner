...
class PionerDB {
  ...
  Future<List<Address>> getAllAddresses() async{
    ...
    //Получаем список всех пользователей
    List<Map<String, dynamic>> query = await dbConnection.mappedResultsQuery('SELECT * from address');
    print(query);
    Address _address;
  ...
  Future<List<Address>> getAllAddressesByOrganizationID(int organization_id) async{
    ...
    //Получаем список всех пользователей
    List<Map<String, dynamic>> query = await dbConnection.mappedResultsQuery('SELECT * from address where organization_id = $organization_id');
    print(query);
    Address _address;
    List<Address> _addresses = [];
    for (var element in query) {
